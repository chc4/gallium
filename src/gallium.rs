...
impl<'a> Gallium<'a> {
    ...
    fn dispatch_key(&mut self, key: Key){
        ...
        let screen = self.window_manager.screens.index.unwrap() as u32;
        for k in self.config.current().keys { if k.binding.unwrap() == key {
            match k.message {
                ...
                //Message::Bring(dir) => {
                //    let mang = self.window_manager;
                //    let w_l = mang.workspaces.cards.len();
                //    let w_i = mang.workspaces.index.unwrap();
                //    let cycle = Cycle::new(w_l);
                //    match dir {
                //        Direction::Backward => {
                //            win.map(|win|{
                //            });
                //            mang.switch(&mut self.window_server,
                //                        self.config.current(),
                //                        cycle[(w_i as isize) - 1].unwrap() as u32);
                //        },
                //        Direction::Forward => {
